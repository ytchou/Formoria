 ...
 * `banner` entry currently reads `"(max-width: 1280px) 100vw, 1280px"`. That
 * 1280px IS today's 80rem content measure, arrived at independently, with
 * nothing in either file mentioning the other — so widening the measure would
 * have left every banner image fetching a crop one size too small, silently and
 * invisibly. Removing that disconnect is the whole reason for this constant.
 *
 * `image.tsx` is deliberately NOT rewired here: that changes which file a
 * browser downloads, which is a rendering change, and this wave ships none.
 *
