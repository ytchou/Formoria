...
/** Kinds the repair agent cannot resolve. */
const AGENT_CANNOT_REPAIR = new Set([
  'dependencies',
  'devDependencies',
  'optionalPeerDependencies',
  'unlisted',
...
/** Kinds that produce report_only findings. */
  ...
  for (const issue of value.issues) {
    ...
    for (const kind of KNIP_KINDS) {
      ...
      for (const [index, entry] of entries.entries()) {
        ...
        const agentCannotRepair = AGENT_CANNOT_REPAIR.has(kind)
        ...
        const disposition: HealthFindingDisposition | undefined =
          REPORT_ONLY_KINDS.has(kind) ? 'report_only' : undefined
        findings.push(
          qualityFinding({
            changedFiles,
            evidence: {
