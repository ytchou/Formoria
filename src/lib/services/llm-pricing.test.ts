...
describe("selectPrice", () => {
  const older: PriceRow = {
    ...LUNA,
    input_per_m: 1,
    effective_from: "2026-01-01T00:00:00Z",
  };
  const newer: PriceRow = {
    ...LUNA,
    input_per_m: 2,
  ...
  it("picks the newest rate not in the future relative to the call", () => {
    expect(
      selectPrice([older, newer], "gpt-5.6-luna", new Date("2026-03-01"))
        ?.input_per_m,
    ).toBe(1);
    expect(
      selectPrice([older, newer], "gpt-5.6-luna", new Date("2026-07-01"))
        ?.input_per_m,
    ).toBe(2);
  });
  ...
  it("returns null for a model with no price row", () => {
    expect(
      selectPrice([older], "deepseek-v4-flash", new Date("2026-07-01")),
    ).toBeNull();
  });
});
...
describe("usageFromRawResponse", () => {
  it("reads the hoisted usage on the audit envelope", () => {
    expect(
      usageFromRawResponse({ usage: { prompt_tokens: 5 } })?.prompt_tokens,
    ).toBe(5);
  });

  it("falls back to the nested response usage", () => {
    expect(
      usageFromRawResponse({ response: { usage: { prompt_tokens: 7 } } })
        ?.prompt_tokens,
    ).toBe(7);
  });
