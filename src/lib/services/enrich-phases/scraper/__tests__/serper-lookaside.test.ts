...
const AVATAR = 'https://scontent-tpe1-1.cdninstagram.com/v/t51.2885-19/39902791_1500267353450836_4606187042949300224_n.jpg?stp=dst-jpg_s150x150_tt6'
...
describe('lookaside image resolution', () => {
  ...
  it('resolves a lookaside url to the uncropped photo rather than the cropped og:image', async () => {
    vi.stubGlobal(
      ...
      vi.fn(async (input: unknown) => {
        ...
        if (url.includes('serper.dev')) return serperResponse([lookasideImage('111')])
        if (url.includes('lookaside.instagram.com')) return htmlResponse(crawlerHtml())
  ...
  it('drops the candidate when the crawler page cannot be fetched', async () => {
    vi.stubGlobal(
      ...
      vi.fn(async (input: unknown) => {
        const url = String(input)
        if (url.includes('serper.dev')) return serperResponse([lookasideImage('222')])
        if (url.includes('lookaside.instagram.com')) return new Response('nope', { status: 500 })
        throw new Error(`Unexpected fetch ${url}`)
      }),
    )
  ...
  it('drops the candidate when the page has no og:image', async () => {
    vi.stubGlobal(
      ...
      vi.fn(async (input: unknown) => {
        const url = String(input)
        if (url.includes('serper.dev')) return serperResponse([lookasideImage('333')])
        if (url.includes('lookaside.instagram.com')) return htmlResponse('<html><head></head></html>')
        throw new Error(`Unexpected fetch ${url}`)
      }),
    )
  ...
  it('still keeps ordinary non-lookaside images untouched', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: unknown) => {
        const url = String(input)
        if (url.includes('serper.dev')) {
          return serperResponse([{ imageUrl: direct, imageWidth: 640, imageHeight: 480 }])
        }
        throw new Error(`Unexpected fetch ${url}`)
      }),
    )
    ...
    expect(outcomes.get('Yogasana')?.rows.map((row) => row.url)).toEqual([direct])
  })
})
