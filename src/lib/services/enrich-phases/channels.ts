...
function normalizeBranchIdentity(
  location: Pick<PhysicalRetailLocation, "name" | "city" | "venueName">,
): string {
  return [
    normalizeChannelName(location.name),
    normalizeText(location.city),
    normalizeText(location.venueName),
  ].join("|");
}
...
function relaxedFallbackRetailerNamesMatch(
  candidateName: unknown,
  placeTitle: unknown,
): boolean {
  const candidate = normalizeText(candidateName);
  ...
  return (
    ...
    titleCore.includes(candidateCore)
  );
}
...
function officialOrigins(
  brand: EnrichBrand,
  scrapedData: EnrichScrapedData | null | undefined,
): Set<string> {
  return new Set(
    [
    ...
    auditResultIds: [
      ...new Set([
        ...
        ...evidence.flatMap((item) =>
          item.auditResultId ? [item.auditResultId] : [],
        ),
      ]),
    ],
    origin,
...
function isIncompletePhysicalLocation(
  location: PhysicalRetailLocation,
): boolean {
  return (
...
async function getExistingChannelCandidates(
  ...
  return (data ?? []).map((row) => {
    ...
    const cityEntry = row.region_label
      ? Object.entries(CITY_REGION_LABELS).find(
          ([, label]) => label === row.region_label,
        )
      : undefined;
    ...
    const location: PhysicalRetailLocation = {
      ...
      confirmationStatus:
        row.owner_status === "confirmed" ? "owner_confirmed" : "unconfirmed",
    };
...
function samePhysicalCandidate(
  left: LocationCandidate,
  right: LocationCandidate,
): boolean {
  if (
    !isPhysicalRetailLocation(left.location) ||
    !isPhysicalRetailLocation(right.location)
  ) {
    return false;
  }
  if (
...
function normalizedNameMatch(
  ...
  const parts =
    brandName
      ...
      ?.map(normalizeText) ?? [];
  if (parts.length > 1 && parts.every((part) => title.includes(part)))
    return true;
  return parts
    .filter((part) =>
      /^\p{Script=Han}+$/u.test(part) ? part.length >= 3 : part.length >= 4,
    )
    .some((part) => title.includes(part));
}

function hasConflictingNamedCity(
  ...
  const expectedCity =
    CITY_NAME_VARIANTS.find(({ names }) =>
      names.some((variant) => targetName.includes(variant)),
...
function inferCityFromAddress(
  address: string,
): (typeof CITY_NAME_VARIANTS)[number]["slug"] | undefined {
  ...
  if (normalizedAddress.includes(normalizeText("新竹市")))
    return "hsinchu_city";
...
function isClearlyNonRetailPlace(place: SerperMapPlace): boolean {
  ...
  return CLEARLY_NON_RETAIL_NAMES.some((marker) =>
...
function getFallbackPlaces(
  ...
  const eligible = result.places.filter(
    (place) => optionalText(place.address) && !isClearlyNonRetailPlace(place),
  );
  ...
  const retailerMatches = eligible.filter((place) =>
    retailerNamesMatch(target.location.name, place.title),
  );
  ...
  if (relaxedRetailerMatches.length > 0)
    return uniqueAddressPlaces(relaxedRetailerMatches);
  return uniqueAddressPlaces(
    eligible.filter(
      (place) =>
        !hasConflictingNamedCity(target, place) &&
        normalizedNameMatch(brandName, place.title),
    ),
  );
}
...
function mapPlaceLocation(
  ...
  const physicalCandidate =
    candidate && isPhysicalRetailLocation(candidate.location)
      ? candidate.location
      : undefined;
  ...
  return {
    ...
    address,
    ...(city ? { city } : {}),
    ...(physicalCandidate?.venueName
      ? { venueName: physicalCandidate.venueName }
      : {}),
    ...(physicalCandidate?.floorOrCounter
      ? { floorOrCounter: physicalCandidate.floorOrCounter }
      : {}),
    ...(optionalCoordinate(place.latitude) !== undefined
      ? { latitude: optionalCoordinate(place.latitude) }
      : {}),
    ...(optionalCoordinate(place.longitude) !== undefined
      ? { longitude: optionalCoordinate(place.longitude) }
      : {}),
    verificationStatus: "needs_review",
...
function getRetailChainCandidate(
  ...
  const distinctAddresses = new Set(
    branchPlaces.map((place) => normalizeLocationAddress(place.address)),
  );
  if (distinctAddresses.size < 2) return null;

  const retailerUrl =
    getSharedRetailerUrl(branchPlaces) ?? getCandidateUrl(target);
  ...
  return {
    ...
    matchReason:
      "Maps lookup returned multiple matching branches with distinct addresses",
    evidence,
    auditResultIds: [
      ...new Set([
...
function applyFallbackPlace(
  ...
  const index = candidates.findIndex((candidate) =>
    samePhysicalCandidate(candidate, target),
  );
  ...
  next[index] = {
    ...
    evidence: [
      ...
      ...(result.auditResultId
        ? [{ source: "maps" as const, auditResultId: result.auditResultId }]
        : []),
    ],
    auditResultIds: [
      ...new Set([
        ...target.auditResultIds,
        ...(result.auditResultId ? [result.auditResultId] : []),
      ]),
    ],
    lookupAttempted: true,
...
function markLookupAttempt(
  ...
  if (!current || !isPhysicalRetailLocation(current.location))
    return candidates;
  ...
  const attemptedLocation: PhysicalRetailLocation = {
    ...current.location,
    verificationStatus:
      current.decision === "verified" ? "verified" : "needs_review",
  };
  const next = [...candidates];
  next[index] = {
    ...current,
...
function applyMapsMatches(
  ...
  for (const place of result.places) {
    ...
    const candidateIndex = next.findIndex((candidate) => {
      ...
      if (!normalizedNameMatch(brandName, place.title, candidate.location.name))
        return false;
      const nameMatches = retailerNamesMatch(
      ...
      const addressMatches = Boolean(
        ...
        placeAddress === candidate.normalizedAddress,
      );
...
function getLocationRegionLabel(
  location: PhysicalRetailLocation,
): string | undefined {
  const city =
    location.city ??
    (location.address ? inferCityFromAddress(location.address) : undefined);
  return city ? (CITY_REGION_LABELS[city] ?? city) : undefined;
}

function getCategoryLabel(
  location: Pick<PhysicalRetailLocation, "relationshipType">,
): string | undefined {
...
): PhysicalRetailLocation["relationshipType"] {
  return (
    (["brand_store", "department_counter", "stockist"] as const).find(
      (relationshipType) =>
        getCategoryLabel({ relationshipType }) === categoryLabel,
    ) ?? "stockist"
  );
}
...
function getCandidateUrl(candidate: LocationCandidate): string | undefined {
  ...
  const evidenceUrl =
    candidate.evidence.find((item) => item.source === "maps" && item.url)
      ?.url ??
    candidate.evidence.find((item) => item.source === "existing" && item.url)
      ?.url;
...
function toChannelCandidates(
  ...
  for (const candidate of candidates) {
    ...
    const shouldReplace =
      ...
      (current.regionLabel !== CHAIN_REGION_LABEL &&
        channel.regionLabel === CHAIN_REGION_LABEL);
    if (shouldReplace) {
      byNormalizedName.set(channel.normalizedName, channel);
    } else if (!current.url && channel.url) {
      byNormalizedName.set(channel.normalizedName, {
        ...current,
        url: channel.url,
      });
    }
...
async function resolveChannelIds(
  ...
  const normalizedNames = [
    ...new Set(candidates.map((candidate) => candidate.normalizedName)),
  ];
  const { data, error } = await options.supabase
    .from("brand_channels")
  ...
  return new Map(
    (data ?? []).flatMap((row) =>
      row.id && row.normalized_name
        ? [[row.normalized_name, row.id] as const]
        : [],
    ),
  );
}
...
export async function runChannelsPhase(
  ...
  const { result, durationMs } = await timePhase(async () => {
    ...
    const broadQuery = [getDisplayBrandName(options.brand), options.brand.city]
      .filter(Boolean)
      .join(" ");
    ...
    const needsFallback = (candidate: LocationCandidate) =>
      ...
      isIncompletePhysicalLocation(candidate.location);
    ...
    const fallbackResults: BrandMapsSearchResult[] = new Array(
      unresolved.length,
    );
    const unresolvedQueue = unresolved.map((candidate, index) => ({
      candidate,
      index,
    }));
    const worker = async () => {
    ...
    await Promise.all(
      Array.from(
        { length: Math.min(FALLBACK_CONCURRENCY, unresolved.length) },
        () => worker(),
      ),
    );
    for (const [index, fallbackResult] of fallbackResults.entries()) {
      ...
      const chainCandidate = getRetailChainCandidate(
        target,
        fallbackResult,
        fallbackPlaces,
      );
      ...
      const rejected = rejectClearlyNonRetailTarget(
        candidates,
        target,
        fallbackResult,
      );
      candidates =
        rejected === candidates
          ? markLookupAttempt(candidates, target, fallbackResult)
          : rejected;
    }
    ...
    const candidatesForUpsert = candidates.filter(
      (candidate) =>
        ...
        (isPhysicalRetailLocation(candidate.location) &&
          Boolean(optionalText(candidate.location.address))),
    );
    ...
    if (
      ...
      const upsertResult = await upsertEnrichedChannels(
        options.target.id,
        channelCandidates,
      );
      if (!upsertResult.ok) {
        throw new Error(
          "Failed to upsert enriched channels: " + upsertResult.code,
        );
      }
    }
    const channelIds = await resolveChannelIds(
      options,
      toChannelCandidates(candidates),
    );
    await persistChannelCandidates(options, candidates, channelIds);
    const patch =
      channelCandidates.length > 0 ? { channels: channelCandidates } : {};
