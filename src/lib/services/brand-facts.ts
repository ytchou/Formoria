...
export async function extractBrandFacts(
  ...
  try {
    for (let attemptIndex = 0; attemptIndex < 2; attemptIndex += 1) {
      ...
      if (!content) {
        console.error(`  → brand facts: empty response, data=${JSON.stringify(data).slice(0, 200)}`)
        return { result: null, attempts, calls }
      }
      ...
      if (!parsed) {
        ...
        return { result: { ...EMPTY_FACTS, rawResponse: data }, attempts, calls }
      }
    ...
    console.error(`  → brand facts failed: ${err instanceof Error ? err.message : err}`)
    return null
  }
}
