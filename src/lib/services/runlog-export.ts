    ...
    return [
      {
        ...
        ...(typeof source.error === "string"
          ? { error: sanitizeJobError(source.error) }
          : {}),
...
function targetLabel(
  target: CurationJobTarget | undefined,
): string | undefined {
...
export function aiEvent(
  ...
  const error =
    typeof row.audit_error === "string"
      ? sanitizeJobError(row.audit_error)
      : undefined;
  ...
  return {
    ...
    ...(targetLabel(target)
      ? { labels: { target: targetLabel(target)! } }
      : {}),
    ...(error ? { error } : {}),
  };
}
...
export function searchEvent(
  ...
  const status =
    callStatus === "succeeded"
      ? "ok"
      : callStatus === "empty" || callStatus === "started"
        ? "warning"
        : "error";
  const labels = {
...
function aggregatePhaseStatus(
  ...
  if (
    ...
    events.some((event) => event.status === "ok")
  )
    return "succeeded";
...
async function queryByJob(
  ...
  if (error) {
    if (isMissingJobIdColumn(error))
      return { rows: [], jobIdColumnMissing: true };
    throw error;
  }
...
export async function exportJobRunLog(
  ...
  const startedAt =
    job.started_at ?? job.created_at ?? new Date(0).toISOString();
  ...
  if (directAiRows.length === 0 && aiRows.length > 0)
    gaps.push(
      "AI audit rows were loaded through the legacy target/time-window fallback",
    );
  if (directSearchRows.length === 0 && searchRows.length > 0)
    gaps.push(
      "Search audit rows were loaded through the legacy target/time-window fallback",
    );
  ...
  const resultsByPhase = new Map<
    string,
    Array<{ target: CurationJobTarget; result: PhaseResult }>
  >();
  ...
  for (const row of searchRows) {
    ...
    searchEventsByPhase.set(name, [
      ...(searchEventsByPhase.get(name) ?? []),
      searchEvent(row, targetById),
    ]);
  }

  const phaseNames = new Set([
    ...resultsByPhase.keys(),
    ...aiEventsByPhase.keys(),
    ...searchEventsByPhase.keys(),
  ]);
  const orderedPhaseNames = [...phaseNames].toSorted((left, right) => {
    ...
    const rightIndex = PHASE_ORDER.indexOf(
      right as (typeof PHASE_ORDER)[number],
    );
    return (
  ...
  const phases: Phase[] = orderedPhaseNames.map((name, index) => {
    ...
    const phaseTokens = (aiEventsByPhase.get(name) ?? []).reduce<
      TokenUsage | undefined
    >((total, event) => addTokens(total, event.tokens), undefined);
    ...
    return {
      ...
      summary:
        entries.length > 0
          ? `Summed across ${entries.length} targets`
          : undefined,
      durationMs,
  ...
  const durationMs = job.started_at
    ? Math.max(
        0,
        new Date(job.completed_at ?? Date.now()).getTime() -
