...
describe("resolveWritablePatch", () => {
  ...
  it("owner and admin both write a previously admin-stamped field", () => {
    ...
    expect(
      resolveWritablePatch({ description: "x" }, s, { source: "owner" })
        .allowed,
    ).toEqual({
      description: "x",
    });
    expect(
      resolveWritablePatch({ description: "x" }, s, { source: "admin" })
        .allowed,
    ).toEqual({
      description: "x",
    });
...
describe("resolveRefreshEnrichmentPatch", () => {
  ...
  it("removes an owner field from _cleared_fields", () => {
    expect(
      ...
      skipped: [
        { field: "reputation_summary", reason: "cleared:protected:owner" },
      ],
    });
  });
