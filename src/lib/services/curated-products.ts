 ...
 */
export const CURATED_PRODUCT_READ_SELECT = `
 ...
 * (`source_checked_at`), and at least one `curated_product_sources` row exists.
 * A product that cannot prove itself never reaches TypeScript.
 *
 * Returns `[]` for a brand with nothing curated.
 */
