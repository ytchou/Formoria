 ...
 */
function makeTagFixture(product_tags: string[], product_tags_en: string[]): string {
...
describe('parseBrandFactsResult', () => {
  it('returns empty facts when the LLM response is not valid JSON', () => {
    const result = parseBrandFactsResult('抱歉，我無法解析，但這裡有超過二十個字元的原始輸出內容')
  ...
  it('parses mit_indicators only when evidence backs the claim', () => {
    const withMit = (mit: unknown) =>
      parseBrandFactsResult(
        JSON.stringify({ product_tags: [], product_tags_en: [], mit_indicators: mit }),
      ).mitIndicators

    expect(withMit({ mentioned: true, evidence: ['台灣製造'], confidence: 'high' })).toEqual({
      mentioned: true,
      evidence: ['台灣製造'],
      confidence: 'high',
    })
...
describe('parseBrandFactsResult — listing verdict', () => {
  ...
  it('degrades a malformed listing value to undefined without throwing', () => {
    expect(parseBrandFactsResult(withListing('reject')).listing).toBeUndefined()
    expect(parseBrandFactsResult(withListing(['reject'])).listing).toBeUndefined()
...
describe('parseBrandFactsResult — product_type', () => {
  ...
  it('parses a valid L1 category slug', () => {
    expect(parseBrandFactsResult(withProductType('beauty')).productType).toBe('beauty')
  })
