...
describe('locale prefixes come from the i18n routing config', () => {
  it('strips every configured locale', () => {
    for (const locale of routing.locales) {
      expect(stripLocalePrefix(`/${locale}/brands/mizu-tw`)).toBe('/brands/mizu-tw')
  ...
  it('classifies every configured locale into one resource', () => {
    ...
    expect(new Set([...ids, classifyRoute('/brands/mizu-tw').resourceId]).size).toBe(1)
  })
})
...
describe('list resource ids fold only resource-defining params', () => {
  ...
  it('ignores tracking params carried by a shared link', () => {
    ...
    expect(classifyRoute('/brands', 'utm_source=newsletter').resourceId).toBe(base)
  ...
  it('still separates real filter and pagination views', () => {
    const base = classifyRoute('/brands').resourceId
    const filtered = classifyRoute('/brands', 'category=home-living').resourceId
    const paged = classifyRoute('/brands', 'category=home-living&page=2').resourceId
  ...
  it('keeps a filter view stable when tracking params ride along with it', () => {
    expect(classifyRoute('/brands', 'category=home-living&_rsc=1a2b3').resourceId).toBe(
      classifyRoute('/brands', 'category=home-living').resourceId,
    )
  })
})
...
describe('non-public surfaces are outside the enumeration ladder', () => {
  ...
  it('leaves every public family accounted', () => {
    for (const path of ['/brands', '/brands/mizu-tw', '/categories', '/about', '/api/search']) {
...
describe('visitor identity eligibility', () => {
  it('never mints on an image request', () => {
    ...
    expect(shouldMintVisitorIdentity('/zh-TW/i/brands/abc/hero.webp')).toBe(false)
  })
