...
describe('story component map — v2 type roles', () => {
  ...
  it('maps markdown headings onto the v2 roles', () => {
    expect(markup(map.h1({ children: 'Heading' }))).toContain('type-page-title')
    expect(markup(map.h2({ children: 'Heading' }))).toContain('type-section')
    expect(markup(map.h3({ children: 'Heading' }))).toContain('type-card-title')
  ...
  it('keeps author classes rather than overwriting them', () => {
    expect(markup(map.h2({ children: 'x', className: 'text-center' }))).toContain(
      'text-center',
    )
  })
  ...
  it('carries no v1 palette class on any mapped element', () => {
    ...
    const rendered = [
      ...withChildren.map((tag) =>
        markup((map[tag] as unknown as (props: object) => ReactElement)({ children: 'x' })),
      ),
      ...voids.map((tag) =>
        markup((map[tag] as unknown as (props: object) => ReactElement)({ alt: '' })),
      ),
    ].join('\n')
...
describe('story component map — trail section numbers', () => {
  ...
  it('numbers nothing on a story, which declares no sections', () => {
    const html = markup(createStoryComponentMap().h2({ children: '先讓光線到位' }))
