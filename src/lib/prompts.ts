...
/**
 * The twelve ratified materials, slug first. Interpolated from `MATERIALS` for
 * the same reason `CATEGORY_LIST` is interpolated from `L1_CATEGORIES`: the
 * vocabulary is CLOSED and mirrored by a Postgres CHECK, so a hand-typed copy
 * here would ask the model for values the write path rejects with a 23514.
 *
 * The Chinese gloss rides along because the model reads a zh-TW product page,
 * but the value it must RETURN is the English slug —
 * `createCuratedProduct`'s material normalisation resolves slugs only and drops
 * a Chinese label silently.
 */
...
subcategories（0-3 個，只能填下列詞彙表中的中文標籤，優先選 category 分支下的詞彙）：
${SUBCATEGORY_VOCAB_BLOCK}
...
{"products":[{"name_zh":"商品的中文名稱","name_en":"English product name 或 null","category":"類別 slug 或 null","subcategories":["詞彙表中的中文標籤"],"material":["材質 slug"],"official_url":"這件商品的商品頁網址","image_source_url":"圖片所在頁面的網址或 null","product_description_zh":"60-160 字耐久事實描述","sources":[{"url":"你讀到事實的頁面網址","source_type":"official|press|retailer|other","claim_zh":"這個來源支持的事實，一句話或 null"}]}]}

## 驗證檢查（輸出前自行確認）
- [ ] products 是否最多 5 筆，且每一筆都是單一商品，而不是分類頁或商品列表頁？
- [ ] category 與 material 是否只填上列 slug？subcategories 是否只填詞彙表中的中文標籤？
