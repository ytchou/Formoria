import { getCurationJobCounts, listCurationJobs } from '@/lib/services/curation-jobs'
import type {
  ExecutiveDateWindow,
...
function assertResult<T>(result: { data: T | null; error: { message?: string } | null }): T {
  if (result.error) throw new Error(result.error.message ?? 'Executive data query failed')
...
function countNetConfirmations(rows: SubscriberRow[], window: ExecutiveDateWindow): number {
...
export function summarizeLocalEngagement(
  ...
  const topBrands = Array.from(totals.entries())
    .flatMap(([id, total]) => {
      ...
      return brand ? [{ id, name: brand.name, slug: brand.slug, ...total }] : []
    })
  ...
  for (const row of links) {
    destinations.set(row.destination, (destinations.get(row.destination) ?? 0) + row.clicks)
  }

  return {
    topBrands,
    destinationMix: Array.from(destinations, ([destination, clicks]) => ({ destination, clicks }))
...
export function summarizeExecutiveBusinessData(
  ...
  return {
    ...
    engagement: summarizeLocalEngagement(input.brands, input.analytics, input.links),
    curation: {
...
async function loadBusinessData(windows: ExecutiveDateWindows): Promise<FormoriaExecutiveBusinessData> {
  const client = createServiceClient()
  const [brandsResult, ownersResult, subscribersResult, analyticsResult, linksResult, jobCounts, jobs] =
    await Promise.all([
      client.from('brands').select('id, name, slug, approved_at').eq('status', 'approved'),
      ...
      listCurationJobs({ view: 'history', limit: 1 }),
    ])
