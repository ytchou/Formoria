...
export function enrichedDataFromDb(json: Record<string, unknown>): EnrichedData {
  return {
    ...
    ...(typeof json.brand_highlights === 'string' ? { brandHighlights: json.brand_highlights } : {}),
...
export function enrichedDataToDb(data: EnrichedData): Record<string, unknown> {
  ...
  if (data.brandHighlights !== undefined) result.brand_highlights = data.brandHighlights
