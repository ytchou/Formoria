...
describe("message catalogue parity", () => {
  ...
  it("owns trail card labels in the trails namespace", () => {
    ...
    for (const catalogue of [zhTW, en]) {
      ...
      expect(catalogue.landing.selectedProducts).not.toHaveProperty(
        oldEyebrowKey,
      );
  ...
  it("message_parity_holds_for_new_taxonomy_keys", () => {
    ...
    expect(
      Object.keys(zhTW.categories.l1).filter((key) => !l1Slugs.has(key)),
    ).toEqual([]);
    expect(
      Object.keys(zhTW.categories.l2).filter((key) => !l2Slugs.has(key)),
    ).toEqual([]);
  });
  ...
  it("message_parity_holds_without_a_materials_block", () => {
    ...
    expect(Object.keys(zhTW.categories).sort()).toEqual(
  ...
  it("en_messages_carry_no_han_keys", () => {
    ...
    const hanKeys = flatten(en.categories).filter((key) =>
      /[\u4e00-\u9fff]/.test(key),
    );
  ...
  it("llms_txt_has_a_description_for_every_l1", () => {
    ...
    expect(
      missing,
      `L1s with no llms.txt description: ${missing.join(", ")}`,
    ).toEqual([]);
    ...
    expect(
      Object.keys(en.categories.descriptions).filter(
        (slug) => !l1Slugs.has(slug),
      ),
    ).toEqual([]);
  });
