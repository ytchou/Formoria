...
describe('soft rate limiting for internal router traffic', () => {
  it.each([
  ...
  ])('does not challenge a user when Next.js identifies a prefetch through %s', async (
    _signal,
    routerHeaders,
    clientIp,
  ) => {
    ...
    for (let requestNumber = 0; requestNumber < 31; requestNumber += 1) {
      reachedLimit = await checkSoftRateLimit(new NextRequest('https://formoria.com/brands/maison-de-taiwan', {
        headers: { 'cf-connecting-ip': clientIp },
      }))
    }
    expect(reachedLimit).toBe(true)

    const response = await proxy(new NextRequest('https://formoria.com/brands/%E0%A4', {
      headers: {
        host: 'formoria.com',
