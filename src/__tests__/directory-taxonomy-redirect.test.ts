...
describe('retired L1 taxonomy redirects', () => {
  ...
  it('reparented_l2_keeps_its_slug_under_the_new_parent', async () => {
    ...
    expect(REPARENTED_L2.filter(([, parent]) => parent === 'kids')).toHaveLength(10)
    expect(REPARENTED_L2.filter(([, parent]) => parent === 'pets')).toHaveLength(7)
    ...
    for (const [slug, parent] of REPARENTED_L2) {
      ...
      expect(ruleFor(rules, `/zh-TW/categories/kids-pets/${slug}`)).toMatchObject({
        destination,
        permanent: true,
      })
    }
