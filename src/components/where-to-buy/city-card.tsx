  ...
  return (
    <article className="border-t border-border py-6">
      <div className="flex items-baseline justify-between gap-4">
        ...
        <span className="text-sm text-muted-foreground">{summary.count} {locationLabel}</span>
      </div>
      <ul className="mt-3 flex flex-wrap gap-2">
        {summary.districts.map((district) => (
          <li key={district.slug}>
            <Link href={`${cityPath}#${district.slug}`} className="inline-flex min-h-8 items-center rounded-full border border-border px-3 text-sm hover:border-primary hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-primary">
