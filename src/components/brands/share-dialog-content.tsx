...
type ChannelKey = Extract<
  ShareChannel,
  "line" | "threads" | "facebook" | "instagram"
>;
...
export function ShareDialogContent({
  ...
  const handleThreadsShare = () => {
    ...
    openShareWindow(
      `https://www.threads.net/intent/post?text=${encodeURIComponent(`${brandName} ${shareUrl}`)}`,
    );
  };
  ...
  return (
    ...
    // className here.
    // `data-ph-no-autocapture` SITS ON THE POPUP, not on a control. The
    // primitive owns the close button now and carries no suppression of its
    // own, and PostHog matches `css_selector_ignorelist` against the clicked
    // element AND its ancestors — so the attribute here is the only place a
    // call site can silence the close. It also covers the channel buttons,
    // which is correct: every one of them already reports itself through
    // `trackBrandPageShared`, and an autocaptured `$click` beside that is a
    // second event for one share.
    <DialogContent size="compact" data-ph-no-autocapture>
