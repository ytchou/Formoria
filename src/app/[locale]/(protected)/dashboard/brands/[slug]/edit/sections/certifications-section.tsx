      ...
      <div className="space-y-3 rounded-lg border border-border bg-card p-4">
        {fields.map((field, index) => (
          <div key={field.id} className="grid gap-2 rounded-lg border border-border p-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,0.8fr)_minmax(0,0.5fr)_minmax(0,1fr)_48px]">
            <Input className="min-h-12 bg-card" placeholder={t('fieldCertificationNamePlaceholder')} {...form.register(`certifications.${index}.name`)} />
            <Input className="min-h-12 bg-card" placeholder={t('fieldCertificationIssuerPlaceholder')} {...form.register(`certifications.${index}.issuer`)} />
            <Input className="min-h-12 bg-card" type="number" min={1900} max={new Date().getFullYear()} placeholder={t('fieldCertificationYearPlaceholder')} {...form.register(`certifications.${index}.year`)} />
            <Input className="min-h-12 bg-card" type="url" placeholder={t('fieldSourceUrlPlaceholder')} {...form.register(`certifications.${index}.sourceUrl`)} />
