...
export async function submitRecommendation(
  ...
  return runWithAuditContext({}, async () => {
    ...
    const tValidation = (key: string) =>
      tSubmit(key as Parameters<typeof tSubmit>[0]);
    ...
    try {
      ...
      const duplicates = await checkBrandDuplicates(
        parsed.name,
        parsed.website,
      );
      ...
      await submitBrandForReview(
        {
          ...
          submitterEmail:
            parsed.guestEmail?.trim() || buildGuestSubmissionEmail(),
        },
        { useServiceRole: true },
      );
...
export async function submitOwnerDetailedBrand(
  ...
  return runWithAuditContext({}, async () => {
    ...
    try {
      const parsed = submissionWizardSchema
        .extend({
          pdpaConsent: z.literal(true),
          marketingEmailOptIn: z.boolean().default(false),
          turnstileToken: z.string().min(1),
      ...
      if (isDnsResolutionError(err)) {
        console.error(
          "Submit owner detailed brand DNS resolution failure:",
          err,
        );
        return { error: t("unexpected") };
      }
