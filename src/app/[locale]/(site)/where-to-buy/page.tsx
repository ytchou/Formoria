...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: safeLocale, namespace: 'whereToBuy' })
...
export default async function WhereToBuyPage({ params }: PageProps) {
  ...
  const locations =
    (await getStockistDirectory().catch(
  ...
  const countryCounts = Map.groupBy(overseas, (location) => location.country ?? 'XX')
  const districtNames = Object.fromEntries(
    cities.flatMap((city) =>
      city.districts.map((district) => [district.slug, tDistricts(district.slug)]),
    ),
  )
