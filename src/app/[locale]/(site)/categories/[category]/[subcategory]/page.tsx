...
export async function generateMetadata({
  ...
  const resolved = resolveCategoryRouteParams({
    categorySlug,
    subcategorySlug,
  });
  if (!resolved) return {};
  ...
  const landingKey = resolved.subcategory
    ? `l2.${resolved.subcategory.slug}`
    : null;
  const title =
    landingKey && catT.has(`${landingKey}.title`)
  ...
  return (
    <DirectoryView
      ...
      filters={{
        ...
        subcategorySlugs: resolved.subcategory
          ? [resolved.subcategory.slug]
          : [],
      }}
      page={parsed.page}
      sort={parsed.sort}
