...
import { stageAdminBrandReviewImage } from "@/lib/services/admin-brand-review";
import {
...
export async function POST(
  ...
  const brandIdResult = reviewEntityIdSchema.safeParse((await params).id);
  ...
  try {
    ...
    const storagePath = `brands/${brandId}/admin/${crypto.randomUUID()}.webp`;
    ...
    try {
      const image = await stageAdminBrandReviewImage({
        brandId,
        storagePath,
        url: uploaded.url,
