...
/**
 * Rejection paths plus the pure summary, deliberately.
 *
 * Every rejection below is answered by `isAuthorizedMachineCaller` or by body
 * validation BEFORE the route calls `sweepPendingPromotions`, so nothing here
...
describe("POST /api/cron/promote-submission-images", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
  });
  ...
  it("rejects an unknown body key", async () => {
    ...
    // `dry_run` is deliberately NOT accepted here: the scheduled sweep is
    // authoritative, and an auditing dry run is the operator CLI.
  ...
  it("rejects a non-string triggered_by", async () => {
    ...
    const response = await POST(post({ triggered_by: 7 }));
...
describe("buildSweepSummary", () => {
  it("reports promoted, skipped and failed counts", () => {
    const result: PromotionResult = {
      plan: {
        ...
        unresolvable: [
          {
            id: "row-unresolvable",
            storagePath: "submissions/broken.webp",
      ...
      conflicts: [
        {
          id: "row-conflict",
          sourceKey: "submissions/s/c.webp",
          targetKey: "brands/b/c.webp",
          kind: "conflict",
        },
      ],
      failures: [
        {
          id: "row-failed",
          sourceKey: "submissions/s/f.webp",
          targetKey: "brands/b/f.webp",
          kind: "failed",
        },
      ],
    };

    expect(buildSweepSummary(result)).toEqual({
      scanned: 9,
      promoted: 6,
      copied: 4,
      adopted: 2,
      ...
      unresolvable: 1,
      conflicts: 1,
      // A conflicting or unresolvable row is still stuck under submissions/ —
      // the same operational fact as an outright failure.
      failed: 3,
    });
  });
});
