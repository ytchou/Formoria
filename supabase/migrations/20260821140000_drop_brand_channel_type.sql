...
--
-- Ordering inside the transaction matters too: the function is replaced BEFORE
-- the column is dropped, because a function body naming a dropped column fails
-- at its next call rather than at drop time.
--
-- Safe to drop rather than rename: production holds 0 rows with
-- channel_type = 'online' out of 1,354, and staging 0 out of 568. Every row is
-- 'offline', so no row's rendering changes.

begin;

-- `create or replace`, never `drop function`. Dropping a public function
-- re-applies Supabase's default privileges to `anon` and `authenticated`, and
-- revoking from `public` afterwards does not undo that — the grants have to be
-- revoked by role name. Replacing in place never loses the revoke.
