-- DEV-1736: expose per-arm rank and raw scores from search_products_semantic so a
-- learned ranker can read them. Fusion, clamps and eligibility are unchanged.
-- Postgres refuses `create or replace` when RETURNS TABLE changes, so drop first.
-- Dropping re-grants execute to PUBLIC; the revoke/grant/assert block below restores
-- the service-role-only contract (see memory: supabase-drop-function-regrants-anon).
