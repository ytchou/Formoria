...
export async function seedBrand(opts: {
  ...
  if (opts.withOwner) {
    ...
    if (!testUser)
      throw new Error("E2E test user not found — check E2E_USER_EMAIL");
  ...
  if (opts.withOwner && testUserId) {
    ...
    if (ownerError) {
      ...
      if (ownerError.code === "23505") {
        console.warn(
          `[e2e-seed] withOwner: user already owns a brand (${ownerError.message}) — continuing without ownership`,
        );
      } else {
        await supabase.from("brands").delete().eq("id", brand.id);
        throw new Error(
          `seedBrand brand_owners insert failed: ${ownerError.message}`,
        );
      }
    }
  }
