...
export async function ensureOwnedBrand(
  ...
  userId: string,
): Promise<{ id: string; slug: string; name: string; draftData: unknown }> {
  const findOwnedBrand = async () => {
    ...
    return { id: brand.id, slug: brand.slug, name: brand.name as string, draftData: brand.draft_data }
  }
  ...
  const brandName = `[E2E-TEST] Dashboard Brand ${suffix}`
  ...
  const { error: ownerError } = await supabase.from('brand_owners').upsert(
    { user_id: userId, brand_id: created.id },
    { onConflict: 'user_id' },
  )

  if (!ownerError) {
    return { id: created.id, slug: created.slug, name: created.name as string, draftData: created.draft_data }
  }
