...
import { ownerFeaturesDisabled, OWNER_FEATURES_ON_REASON } from '../helpers/owner-features'
...
test.describe('Owner features gated off', () => {
  ...
  test('a signed-in visitor is offered no claim CTA on a brand page', async ({ userPage }) => {
    test.setTimeout(90_000)

    const resp = await userPage.goto(`/brands/${brandSlug}`, { timeout: 60_000 })
    if (resp?.status() === 503) {
    ...
    await expect(userPage.getByRole('button', { name: '認領這個品牌' })).toHaveCount(0)
  })
  ...
  test('the owner dashboard and owner submit fork are unreachable for a signed-in non-admin', async ({
    ...
    for (const ownerPath of ['/submit/owner', '/submit/owner/details', '/en/submit/owner']) {
      const response = await userPage.goto(ownerPath, { timeout: 60_000 })
      expect(response?.status(), `${ownerPath} must not be reachable`).toBe(404)
    }
  })

  test('signing in lands on the home page, not the dashboard', async ({ anonPage }) => {
  ...
  test('nav offers the Submit CTA and the account menu has no my-submissions entry', async ({
    userPage,
  }) => {
    test.setTimeout(90_000)
    ...
    const submitCta = header.getByRole('link', { name: '提交品牌' }).first()
    ...
    await expect(accountMenu.getByRole('menuitem', { name: '帳號設定' })).toBeVisible({
      timeout: 10_000,
    })
    await expect(accountMenu.getByRole('menuitem', { name: '我的提交' })).toHaveCount(0)
    await expect(accountMenu.locator('a[href*="my-submissions"]')).toHaveCount(0)
  })
})
