 ...
 *
 * Steps:
 *   1. Seed a controlled approved brand
 *   2. Open its brand detail page
...
test.describe("Brand share dialog", () => {
  ...
  test("dialog shows the url field and four channels", async ({ page }) => {
    ...
    await expect(
      dialog.getByText(brandName, { exact: false }).first(),
    ).toBeVisible();
  ...
  test("instagram copies without opening when the clipboard is denied", async ({
    page,
  }) => {
