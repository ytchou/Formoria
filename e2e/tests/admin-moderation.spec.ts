...
test.describe("Content moderation flow", () => {
  ...
  test.beforeAll(async () => {
    ...
    if (brandError || !brand) {
      throw new Error(
        `Failed to seed moderation brand: ${brandError?.message}`,
      );
    }
    brandId = brand.id;
    ...
    if (imageError) {
      throw new Error(
        `Failed to seed moderation images: ${imageError.message}`,
      );
    }
  });
  ...
  test("admin cannot bypass the block and can review the resulting queue rows", async ({
    ...
    await expect(
      pendingRows.first().getByRole("button", { name: "Mark reviewed" }),
    ).toBeVisible();
    await expect(
      pendingRows.first().getByRole("button", { name: "Dismiss" }),
    ).toBeVisible();
