...
4. **Acceptance:** confirm the next 07:13 run delivers one row for each collector and one complete Slack digest. Confirm operators receive GitHub Actions notifications for an intentionally failed test run. A scheduled run that is never created does not produce a repository-owned alert. Then remove the external Claude Routine; there is no shadow period.
