...
async function main(): Promise<void> {
  ...
  if (!submissionId) throw new Error('usage: enrich-submission.ts <submission-id> [steps...]')

  const requestedSteps = process.argv.slice(3)
  const steps = requestedSteps.length > 0 ? requestedSteps : [...STEPS]
  const unknown = steps.filter((step) => !STEPS.includes(step as (typeof STEPS)[number]))
  if (unknown.length > 0) {
    throw new Error(`unknown step(s): ${unknown.join(', ')} — expected ${STEPS.join(', ')}`)
  }

  const supabase = serviceClient()
  const adminEmail = (process.env.ADMIN_EMAILS ?? '').split(',')[0]?.trim() || 'admin@formoria.com'

  const { data: before, error: beforeError } = await supabase
    .from('brand_submissions')
    .select('id, brand_name, status, intent, brand_id, website_url, enriched_data')
    .eq('id', submissionId)
    .single()
  if (beforeError) throw beforeError

  console.log(`\n[1/3] target: ${before.brand_name}`)
  console.log(`      status=${before.status} intent=${before.intent} brand_id=${before.brand_id ?? 'null'}`)
  console.log(`      enriched_data before: ${before.enriched_data ? `${Object.keys(before.enriched_data).length} key(s)` : 'null'}`)

  console.log(`\n[2/3] enqueueing curation job — steps: ${steps.join(', ')}`)
  const job = await enqueueAdminCurationJob({
    params: { target: 'submissions', submissionIds: [submissionId], steps, overwrite: true },
    dryRun: false,
  ...
  console.log(`      done — success ${summary.success}, failed ${summary.failed}, skipped ${summary.skipped}`)
  ...
  if (target) {
    console.log(`\nphases:`)
    for (const phase of (target.phase_results ?? []) as { phase: string; status: string; note?: string }[]) {
      console.log(`  ${phase.phase.padEnd(18)} ${phase.status}${phase.note ? ` — ${phase.note}` : ''}`)
    }
  ...
  const { data: calls } = await supabase
    ...
    .select('phase, model, prompt_tokens, cached_prompt_tokens, completion_tokens, cost_usd')
  ...
  if (rows.length > 0) {
    ...
    for (const [phase, entry] of [...byPhase].sort((a, b) => b[1].cost - a[1].cost)) {
      console.log(`  ${phase.padEnd(18)} ${String(entry.calls).padStart(3)} call(s)  ${usd(entry.cost)}`)
    }
  } else {
