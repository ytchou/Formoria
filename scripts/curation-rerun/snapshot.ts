...
/**
 * Child tables keyed by brand_id, with the unique column to page by. Paging
 * needs a stable total order or rows shift between pages and the backup both
 * duplicates and drops. `brand_faq` holds one row per brand and has no `id` —
 * its `brand_id` IS the key — so the column cannot be assumed.
 */
const CHILD_TABLES = [
  { table: 'brand_images', orderBy: 'id' },
  { table: 'brand_faq', orderBy: 'brand_id' },
...
async function main(): Promise<void> {
  ...
  const out = resolve(snapshotDir(cohort), argValue('--out') ?? `snapshot-${Date.now()}.json`)
  ...
  const rows = await selectAllPages<Record<string, unknown>>((from, to) =>
    supabase.from('brands').select('*').in('slug', cohort.slugs).order('id').range(from, to)
  )
  if (rows.length !== cohort.slugs.length) {
    ...
    throw new Error(`only ${rows.length}/${cohort.slugs.length} brands found. Missing: ${missing.join(', ')}`)
  }
  ...
  for (const { table, orderBy } of CHILD_TABLES) {
    ...
    try {
      page = await selectAllPages<unknown>((from, to) =>
        supabase.from(table).select('*').in('brand_id', ids).order(orderBy).range(from, to)
      )
    } catch (childError) {
  ...
  await writeFile(
    out,
    JSON.stringify(
      { capturedAt: new Date().toISOString(), cohort: cohort.name, slugs: cohort.slugs, brands: rows, children },
      null,
      2
    )
  )
