...
const INTENT =
  process.argv.find((arg) => arg.startsWith('--intent='))?.slice('--intent='.length).trim() ?? ''
...
type Candidate = { id: string; brandName: string; intent: string; verdict: string }
...
async function loadCandidates(): Promise<Candidate[]> {
  ...
  const latestBySubmission = new Map<string, { status: string; error: string | null }>()
...
async function main(): Promise<void> {
  ...
  const selected = all.filter((row) => ONLY.length === 0 || ONLY.includes(row.id))
  ...
  const bucket = selected.filter((row) => INTENT === '' || row.intent === INTENT)

  console.log(`[reject-skipped] mode: ${APPLY ? 'APPLY' : 'DRY RUN (pass --apply)'}`)
  console.log(
    `[reject-skipped] detection-skipped, still pending: ${bucket.length}` +
      (INTENT ? ` (intent=${INTENT}, ${all.length - bucket.length} other rows left alone)` : ''),
  )
  ...
  for (const [index, row] of bucket.entries()) {
    ...
    const reason = row.verdict.slice(NOT_A_BRAND_PREFIX.length).replace(/^:\s*/, '')
    ...
    try {
      ...
      if (wouldEmail) {
        skipped += 1
        console.log(`${label} — SKIP (real submitter would be emailed; use /admin/submissions)`)
        continue
      }
      if (!APPLY) {
      ...
      failed += 1
      console.error(`${label} — FAILED: ${err instanceof Error ? err.message : JSON.stringify(err)}`)
    }
  }

  console.log(`\n[reject-skipped] rejected: ${rejected}  skipped: ${skipped}  failed: ${failed}`)
  if (rejected > 0) {
    console.log('[reject-skipped] brand rows stay hidden; delete them separately if unwanted.')
  }
}
