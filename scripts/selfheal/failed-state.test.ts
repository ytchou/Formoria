...
describe("bounded failed-test state", () => {
  it("preserves an exact shrinking set from four failures to zero", () => {
    ...
    expect(
      validateCollectedIds(
        { status: "failed", failedTests: collected },
        collected,
      ),
    ).toBe(4);
    expect(
      validateCollectedIds(
        { status: "failed", failedTests: ["c", "d"] },
        collected,
      ),
    ).toBe(2);
    expect(
      validateCollectedIds({ status: "passed", failedTests: [] }, collected),
    ).toBe(0);
  });

  it("fails closed when an edited test ID can no longer be collected", () => {
    expect(() =>
      validateCollectedIds({ status: "failed", failedTests: ["renamed"] }, [
        "current",
      ]),
  ...
  it("keeps missing Claude usage additive and non-blocking", () => {
    ...
    expect(
      parseModelUsage([
        { type: "assistant" },
        { type: "result", total_cost_usd: 2, num_turns: 9, duration_ms: 4000 },
      ]),
