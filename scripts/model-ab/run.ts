...
import { createOpenAIClient, resolveOpenAIModel } from '@/lib/services/openai-client'
...
type EvalBrand = {
  ...
  textPhases: Array<{ phase: string; status: string; changedFields: string[]; durationMs: number; error?: string; detail?: string }>
  imageUsage: Usage
  textUsage: Usage
...
async function gateCandidate(candidate: CandidateImage, claimed: string[]): Promise<EvalImage> {
  const base = {
    url: candidate.url,
  ...
  try {
    ...
    if (isNonImageContentType(res.headers.get('content-type') ?? '')) return { ...base, gate: 'content-type' }
    ...
    if (typeof stats.entropy === 'number' && stats.entropy < MIN_ENTROPY) return { ...sized, gate: 'entropy' }
...
async function classifyBatch(
  ...
  if (!result.ok || !result.content) {
    console.log(`      classify failed: status ${result.status} ${JSON.stringify(result.errorBody).slice(0, 160)}`)
    return new Map()
  }
...
async function main(): Promise<void> {
  const slugs = (argValue('--slugs') ?? '').split(',').map((s) => s.trim()).filter(Boolean)
  const outPath = argValue('--out')
  if (slugs.length === 0 || !outPath) throw new Error('usage: --slugs a,b,c --out path.json')
  if (!process.env.CURATION_EVAL_SINK) {
    throw new Error('CURATION_EVAL_SINK must be set, or LLM audit rows will be written to production')
  }

  const model = resolveOpenAIModel()
  const cacheDir = argValue('--cache')
  const cacheMode = (argValue('--cache-mode') ?? 'off') as 'record' | 'replay' | 'off'
  if (cacheDir && cacheMode !== 'off') {
    // Supabase is exempt: its rows are read-only here and already stable.
    installHttpCache(cacheDir, cacheMode, [new URL(process.env.NEXT_PUBLIC_SUPABASE_URL!).host])
  }

  console.log(`model:  ${model}`)
  console.log(`brands: ${slugs.join(', ')}`)
  console.log(`sink:   ${process.env.CURATION_EVAL_SINK}`)
  console.log(`cache:  ${cacheDir ? `${cacheMode} @ ${cacheDir}` : 'off — inputs will NOT be frozen'}\n`)
  ...
  if (rows.length !== slugs.length) {
    ...
    throw new Error(`missing brand(s): ${slugs.filter((s) => !found.has(s)).join(', ')}`)
  }
  ...
  const slugById = new Map<string, string>(
    syntheticSubmissions.map((s, i) => [String(s.id), String(rows[i]?.slug ?? '')])
  )
  ...
  for (const b of rows) {
    ...
    try {
      if (scrapeSet.length > 0) scraped = (await scrapeBrandUrls(scrapeSet)).data as unknown as Record<string, unknown>
    } catch (e) {
    ...
    const pool = buildCandidatePool({
      scraped: (
        (scraped.imageSources as Array<{ url: string; method: string; pageUrl: string; position: number }>) ?? []
      ).map((s) => ({ url: s.url, method: s.method, pageUrl: s.pageUrl, position: s.position })),
  ...
  // mapping.
  const sinkLines = (await readFile(process.env.CURATION_EVAL_SINK, 'utf8')).trim().split('\n').filter(Boolean)
  const textBySlug = new Map<string, { total: Usage; byPhase: Record<string, Usage> }>()
  for (const line of sinkLines) {
    ...
    if (!slug) continue
    const entry = textBySlug.get(slug) ?? { total: { prompt: 0, completion: 0, calls: 0 }, byPhase: {} }
    const phase = (entry.byPhase[record.phase] ??= { prompt: 0, completion: 0, calls: 0 })
  ...
  await writeFile(
    outPath,
    JSON.stringify({ model, ranAt: new Date().toISOString(), blockedWrites: blocked, brands: results }, null, 2)
  )
...
void main().catch((e) => {
  console.error('\nFAILED:', e instanceof Error ? (e.stack ?? e.message) : JSON.stringify(e))
  process.exitCode = 1
})
