 ...
 */
const QUERIES = ['Entadar 海漂計畫', 'Yates 亞堤斯', '日句時刻所 Heyyou Moment']
const REPEATS = 5

async function search(q: string): Promise<{ links: string[]; snippets: number }> {
  ...
  const json = (await res.json()) as { organic?: Array<{ link: string; snippet?: string }> }
...
async function main(): Promise<void> {
  for (const q of QUERIES) {
    ...
    for (const r of runs) for (const l of new Set(r)) freq.set(l, (freq.get(l) ?? 0) + 1)
    ...
    console.log(`  in exactly 1 run : ${once}  (${((once / union.size) * 100).toFixed(0)}% of union is one-off noise)`)
