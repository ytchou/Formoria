...
#
# The compliance badge grades one objective thing: did the model emit tags the
# storage layer accepts? Since DEV-1510 that means English L2 slugs — a zh-TW
# label in `subcategories` is no longer storable, and the badge reports it as
# non-compliant on purpose.
#
# Read from the GENERATED `taxonomy_terms` seed, never from
# `src/lib/taxonomy/ontology.ts`. The regex that used to live here reported 172
# of the 175 L2 nodes, because three entries wrap onto extra lines — and it
# failed silently, so the badge simply read low and no type checker covered it.
# The seed block is emitted by `scripts/generate-taxonomy-terms.ts`, which
# imports the ontology module instead of parsing it, is gated by
# `taxonomy-terms-parity.test.ts`, and carries its own per-axis count
# assertion. Checking the parse against that assertion turns a silent
# under-count into a hard failure.
ROOT = Path(__file__).resolve().parents[2]
SEED_BEGIN = (
    "-- >>> generated by scripts/generate-taxonomy-terms.ts - do not edit by hand"
)
SEED_END = "-- <<< end generated block"


def load_taxonomy_terms():
    """Returns the (axis, slug, name_zh, name_en) rows of the current seed.

    Migrations are immutable, so a later vocabulary change (DEV-1507 retires
    `crafts`) ships a NEW migration that re-seeds the table. Filenames are
    timestamp-prefixed, so the highest-sorting file carrying the marker is the
    current seed and there is no path to re-point here when that happens.
    """
    seeded = sorted(
        path
        for path in (ROOT / "supabase" / "migrations").glob("*.sql")
        if SEED_BEGIN in path.read_text(encoding="utf-8")
    )
    if not seeded:
        raise SystemExit(
            "No migration carries a generated taxonomy_terms seed block. Run "
            "`pnpm exec tsx scripts/generate-taxonomy-terms.ts --write` first."
        )

    migration = seeded[-1]
    sql = migration.read_text(encoding="utf-8")
    begin = sql.index(SEED_BEGIN)
    block = sql[begin : sql.index(SEED_END, begin)]

    # Only the INSERT is parsed. The count assertion that follows it is plpgsql
    # with its own quoted literals, and a pattern allowed to wander into it
    # would read them as terms.
    values_at = block.index("values")
    body = block[values_at : block.index(";", values_at)]
    ...
    asserted = re.search(r"v_expected constant jsonb := '([^']*)'::jsonb", block)
    if not asserted:
        raise SystemExit(f"{migration.name} carries no taxonomy_terms count assertion")
    expected = json.loads(asserted.group(1))

    actual = {}
    for axis, *_rest in rows:
        actual[axis] = actual.get(axis, 0) + 1
    if actual != expected:
        raise SystemExit(
            f"taxonomy_terms seed parsed as {actual}, but {migration.name} asserts "
            f"{expected}. The vocabulary would be under-counted — fix the parse."
        )
    return rows


VOCAB = {slug for axis, slug, _zh, _en in load_taxonomy_terms() if axis == "l2"}
