...
const BRANDS = [
  ...
  {
    slug: "heyyou-moment",
    query: "日句時刻所 Heyyou Moment",
    own: ["heyyoumoment.com"],
  },
...
async function main(): Promise<void> {
  for (const brand of BRANDS) {
    ...
    console.log("\n" + "=".repeat(100));
    console.log(
      `${brand.slug}   query: ${brand.query}   (run sizes: ${runs.map((r) => r.length).join(", ")})`,
    );
    ...
    for (const [link, r] of rows) {
      ...
      const cells = r
        .map((x) => (x === undefined ? " ." : String(x).padStart(2)))
        .join(" ");
      const m = meta.get(link)!;
      console.log(
        `${mark}   ${cells}  ${kind(link, brand.own).padEnd(7)} ${host(link)}`,
      );
      console.log(`                        ${(m.title ?? "").slice(0, 76)}`);
      console.log(
        `                        ${(m.snippet ?? "").replace(/\s+/g, " ").slice(0, 76)}`,
      );
    }
    const keep = rows.filter(([, r]) => r.filter(Boolean).length >= 2).length;
    console.log("-".repeat(100));
    console.log(
      `union ${rows.length}   ->  kept by ">=2 of 3" filter: ${keep}   dropped: ${rows.length - keep}`,
    );
  }
}
