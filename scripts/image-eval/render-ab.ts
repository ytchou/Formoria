...
type BeforeBrand = Record<string, unknown> & {
  slug: string;
  name: string;
  images: Img[] | null;
};
...
function afterValue(w: WorkerBrand, field: string): unknown {
  const enriched = (w.submission?.enriched_data ?? {}) as Record<
    string,
    unknown
  >;
  if (enriched[field] !== null && enriched[field] !== undefined)
...
async function main(): Promise<void> {
  ...
  const pairs = beforeFile.rows
    .map((b) => ({
      before: b,
      after: worker.brands.find((w) => w.slug === b.slug),
    }))
    .filter((p): p is { before: BeforeBrand; after: WorkerBrand } =>
      Boolean(p.after),
    );
  ...
  const summary = pairs
    .map(({ before: b, after: w }) => {
      ...
      const delta = pub - now;
      const cls =
        pub === 0 ? "bad" : delta > 0 ? "good" : delta < 0 ? "warn" : "";
      return `<tr>
