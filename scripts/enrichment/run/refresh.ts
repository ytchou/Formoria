...
async function main(): Promise<void> {
  ...
  // --reject-stale: reject any existing pending refresh submissions so that
  // step [1/4] creates fresh ones with current base_brand_data. Without this,
  // refresh.ts reuses stale submissions whose base_brand_data no longer matches
  // the brand (because earlier pipeline runs modified it), and
  // apply_brand_refresh rejects them as "Refresh is stale".
  if (hasFlag(argv, "--reject-stale") && brandIds.length > 0) {
    ...
    const staleSubmissions = stale ?? [];
    if (staleSubmissions.length > 0) {
      if (dryRun) {
        for (const s of staleSubmissions)
          console.log(`  would reject: ${s.brand_name}`);
        console.log(
          `[reject-stale] dry run — ${staleSubmissions.length} stale submission(s) not changed`,
        );
      } else {
        const staleIds = staleSubmissions.map((s) => s.id);
        ...
        for (const s of staleSubmissions)
          console.log(`  rejected: ${s.brand_name}`);
        console.log(
          `[reject-stale] rejected ${staleSubmissions.length} stale submission(s)`,
        );
      }
    } else {
      console.log(
        `[reject-stale] no pending submissions for ${cohort.name} (${slugs.length} brands)`,
      );
    }
  }
