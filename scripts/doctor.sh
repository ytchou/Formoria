...
check_env() {
  if [ ! -f ".env.local" ]; then
    ...
    if ! grep -q "PRODUCTION_BASE_URL=https://" .env.local 2>/dev/null; then
      echo "WARN: PRODUCTION_BASE_URL not set — the health agent surface detector needs it"
    fi
    ...
    # EDITORIAL_AGENT (optional): "off" disables the editorial agent
    if grep -q '^EDITORIAL_AGENT=off$' .env.local 2>/dev/null; then
      echo "WARN: EDITORIAL_AGENT=off — the editorial agent is disabled (falls back to individual phase calls)"
    fi
...
check_ops_agent_vars() {
  ...
  for var in SLACK_BOT_TOKEN SLACK_SIGNING_SECRET OPS_AGENT OPS_AGENT_OPERATORS OPS_AGENT_DAILY_CAP OPS_AGENT_RAILWAY_TOKEN OPS_ROUTINE_TOKEN OPS_ROUTINE_ID; do
