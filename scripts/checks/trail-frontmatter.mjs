...
const L1_CATEGORIES = new Set(
  Object.keys(
    JSON.parse(readFileSync(join("messages", "en.json"), "utf8")).categories
      .descriptions,
  ),
);
...
function checkTrail(file, raw) {
  ...
  for (const field of [
    ...
    if (!isNonEmptyString(data[field]))
      fileFailures.push(`${file}: missing \`${field}\``);
  }

  for (const field of ["publishedAt", "reviewedAt", "reviewDueAt"]) {
    if (!isDateLike(data[field]))
      fileFailures.push(`${file}: missing or malformed \`${field}\``);
  }

  if (!LOCALES.has(data.locale)) {
    fileFailures.push(
      `${file}: \`locale\` must be one of ${[...LOCALES].join(", ")}`,
    );
  }
  ...
  if (!Array.isArray(data.sections) || data.sections.length === 0) {
    ...
    data.sections.forEach((section, index) => {
      ...
      if (!key)
        fileFailures.push(
          `${file}: \`sections[${index}].key\` must be non-empty`,
        );
      else if (seenKeys.has(key))
        fileFailures.push(
          `${file}: duplicate section key ${JSON.stringify(key)}`,
        );
      else seenKeys.add(key);
      if (!isNonEmptyString(section?.title)) {
        fileFailures.push(
          `${file}: \`sections[${index}].title\` must be non-empty`,
        );
      } else {
        const title = section.title.trim();
        if (seenTitles.has(title)) {
          fileFailures.push(
            `${file}: duplicate section title ${JSON.stringify(title)}`,
          );
        } else {
  ...
  if (!Array.isArray(data.sources) || data.sources.length === 0) {
    ...
    for (const source of data.sources) {
      if (typeof source !== "string" || !/^https?:\/\//.test(source)) {
        fileFailures.push(
          `${file}: source is not an absolute URL: ${JSON.stringify(source)}`,
        );
      }
    }
  }
  ...
  if (isNonEmptyString(data.heroImage) && data.heroImage.startsWith("/")) {
    if (!existsSync(join(PUBLIC_DIR, data.heroImage))) {
      fileFailures.push(
        `${file}: \`heroImage\` not found on disk: ${PUBLIC_DIR}${data.heroImage}`,
      );
    }
  }
...
for (const file of files) {
  failures.push(
    ...checkTrail(file, readFileSync(join(TRAILS_DIR, file), "utf8")),
  );
}
...
console.log(
  `Trail frontmatter guard passed (${files.length} file(s) scanned).`,
);
