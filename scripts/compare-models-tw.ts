...
import { createDeepSeekClient, parseDeepSeekJson } from '../src/lib/services/deepseek-client'
import { localizeToTW } from '../src/lib/services/taiwan-localization'
import { createOpenAIClient, parseJson } from '../src/lib/services/openai-client'
...
function buildUserContent(input: DescriptionInput, fallbackBrandName: string): string {
  ...
  const snippets = Array.isArray(input.snippets)
    ? input.snippets.filter((snippet): snippet is string => typeof snippet === 'string')
    : []
...
function extractChineseOutput(content: string | null, parser: (value: string) => unknown): ChineseOutput {
  if (!content) return { description: null, blurb: null }

  const parsed = parser(content)
  if (!parsed || typeof parsed !== 'object') return { description: null, blurb: null }
...
async function runDeepSeek(user: string) {
  ...
  const result = await createDeepSeekClient({ model: 'deepseek-v4-flash' }).chat({
...
async function loadInputs(supabase: ReturnType<typeof createServiceClient>) {
  ...
  const brandNames = new Map((brands as BrandRow[] | null ?? []).map((brand) => [brand.id, brand.name]))
...
async function main(): Promise<void> {
  ...
  for (const item of inputs) {
    const input = item.input && typeof item.input === 'object' ? item.input as DescriptionInput : {}
    const user = buildUserContent(input, item.brandName)
    const [deepSeek, openAi] = await Promise.all([runDeepSeek(user), runOpenAI(user)])
    const deepSeekOutput = extractChineseOutput(deepSeek.content, parseDeepSeekJson)
      ...
      deepseek: {
        ...
        localized: {
          description: deepSeekOutput.description
            ? localizeToTW(deepSeekOutput.description, { brandName: item.brandName }).text
            : null,
          blurb: deepSeekOutput.blurb
            ? localizeToTW(deepSeekOutput.blurb, { brandName: item.brandName }).text
            : null,
        },
      },
      'gpt-4o-mini': {
        ...
        localized: {
          description: openAiOutput.description
            ? localizeToTW(openAiOutput.description, { brandName: item.brandName }).text
            : null,
          blurb: openAiOutput.blurb
            ? localizeToTW(openAiOutput.blurb, { brandName: item.brandName }).text
            : null,
        },
      },
    }, null, 2))
