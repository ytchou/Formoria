...
import { getBrandById, syncBrandImages, updateBrand } from '@/lib/services/brands'
...
function describeError(error: unknown): string {
  if (error instanceof Error) {
    return error.cause ? `${error.message} | cause=${JSON.stringify(error.cause)}` : error.message
  }
...
async function main(): Promise<void> {
  ...
  console.log(`[approve-ready] mode: ${APPLY ? 'APPLY' : 'DRY RUN (pass --apply)'}`)
  ...
  const reviewerId = APPLY && bucket.length > 0 ? await resolveReviewerId(adminEmail) : ''
  ...
  for (const [index, row] of bucket.entries()) {
    ...
    try {
      ...
      if (submission.intent === 'refresh') {
        ...
        if (unhidden) await updateBrand(refresh.brandId, { status: 'approved' })
      ...
      if (submission.isBrandOwner) {
        skipped.push(`${row.brandName} — owner submission needs a claim invite email`)
        console.log(`${label} — SKIPPED (needs claim invite; use the admin UI)`)
        continue
      }
      if (!isGeneratedGuestSubmissionEmail(submission.submitterEmail)) {
        skipped.push(`${row.brandName} — would email ${submission.submitterEmail}`)
        console.log(`${label} — SKIPPED (would email a real submitter; use the admin UI)`)
        continue
      }
      ...
      try {
        ...
        console.warn(`${label} — markFlagsReviewed failed: ${describeError(error)}`)
      }
      ...
      if (brand.heroImageUrl) {
        try {
          ...
          if (sync.failed > 0) imageNote = ` (image sync failed: ${sync.failed})`
        } catch (error) {
          console.warn(`${label} — syncBrandImages failed: ${describeError(error)}`)
