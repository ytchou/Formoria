...
const PAGE_ENTRYPOINT = /^src\/app\/(?:.*\/)?page\.(?:ts|tsx|js|jsx)$/;
/**
...
export function isUserFacingPageEntrypoint(file) {
  ...
  return !routePath
    .split("/")
    .some((segment) => ["api", "internal"].includes(segment));
}
...
export function countTestTags(sourceText, tag) {
  const testCasePattern = /\btest(?:\.serial|\.only)?\s*\(\s*(['"])([^\n]*)\1/g;
...
export function routePatternFor(entrypoint) {
  ...
  const segments = routePath
    ...
    .filter(
      (segment) =>
        segment && segment !== "[locale]" && !/^\(.+\)$/.test(segment),
    );
...
export function buildSelectionIndex(files, sourceByFile, fileExists) {
  ...
  const userRouteEntrypoints = files.filter(
    (file) => isUserFacingPageEntrypoint(file) && sourceByFile.has(file),
  );
  const userRoutes = [
...
export function selectSelection(changedFiles, selectionIndex) {
  ...
  return {
    ...
    browser_projects:
      crossBrowserReasons(changedFiles).length > 0
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const selectionIndex = buildSelectionIndex(files, sourceByFile, (file) =>
    trackedSet.has(file),
  );
