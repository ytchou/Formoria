...
function composeResult(
  ...
  for (const p of pairs.slice(MAX_TAGS)) {
    if (!cappedOriginals.has(p.originalTag))
      perTagSource.set(p.originalTag, "dropped");
  }
...
function buildCsvRow(result: BrandResult): string {
  ...
  return [result.brand.slug, beforeZh, afterZh, perTagSource]
    .map(csvField)
    .join(",");
}
...
async function main(): Promise<void> {
  ...
  const results: BrandResult[] = brandPlans.map(({ brand, plan }) => {
    const category = brand.product_type ?? "unknown";
    const llmMapping =
      globalLlmMapping.get(category) ?? new Map<string, string | null>();
  ...
  if (options.dryRun) {
    ...
    for (const result of results) {
      if (JSON.stringify(result.beforeZh) === JSON.stringify(result.afterZh))
        continue;
