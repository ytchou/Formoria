...
The controlled canary is either the single finding with fingerprint
`directory:canary:github-app-pr` or a provider-backed Sentry finding whose
sanitized evidence has `canaryKind` equal to `sentry-real-lifecycle`. For either
canary, replace the contents of `health-agent-canary.txt` with the sanitized
`desiredMarker` value from its evidence. Do not edit any other file for a canary.
